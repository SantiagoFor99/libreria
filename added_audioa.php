...
$idab=$_POST['id_audiobook'];
$name_audio=$_POST['name_audiobook'];
$url=$_POST['url_audiobook'];
$idb=27;
$verify = "select * from audiobooks where name_audiobook='$name_audio'";
$veri= mysqli_query($conexion, $verify);
if($veri){echo '<script> alert("El audio libro ya esta registrado");</script>';
    echo '<script> window.location="add_audio.html"; </script>';}else{
    $consulta = "insert into audiobooks(id_audiobook,name_audiobook,url_audiobook,id_book) values ('$idab','$name_audio','$url','$idb')" ;
}
echo "<br>";
